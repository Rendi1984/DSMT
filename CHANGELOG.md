...
## 3.31.2
**Real bug (the console silently depended on internet access)**: F12 Network showed `react.production.min.js`/`react-dom.production.min.js` stuck pending whenever the machine was offline, with an eventual `[bundle] error`. Root cause: the compiled DC (Data Conductor) framework runtime embedded in `index.html` always calls `https://unpkg.com/react@18.3.1/umd/react.production.min.js` (and the matching `react-dom`) at boot unless `window.React`/`window.ReactDOM` already exist - this was never conditional on Demo/Live mode, it ran on every page load, contradicting this project's "no internet required" console. Fixed by vendoring the exact same React 18.3.1 / ReactDOM 18.3.1 production UMD builds (byte-identical - verified against the runtime's own SRI hashes) as two new offline-embedded assets, loaded via `<script>` tags inserted immediately before the DC runtime's own bootstrap script; the runtime's existing "skip the CDN fetch if `window.React`/`window.ReactDOM` are already set" check now always short-circuits, so no network call to `unpkg.com` happens at all. Verified headless with `unpkg.com` DNS-blackholed - renders identically to a normal run, zero attempted requests to `unpkg.com`.
- Also moved the sign-in-screen version label (added in 3.31.1) from under the Demo/Live toggle buttons to directly under the "LIVE · authenticating against the API" / "demo · any username..." status line, per follow-up clarification.
- Hot-swap: `index.html` -> the IIS webroot, then Ctrl+F5. No service restart needed. This is a console-only fix (no API/PowerShell changes).
- **If the LDAP "Test connection" button (3.31.0) still returns "LDAP test failed: Failed to fetch" or a 405 in F12**: that means the server is still running the *previous* `DSMT_Api.ps1`, which doesn't have the `/api/directory/test` route yet - Pode's catch-all OPTIONS route makes any unmatched path answer preflight OK (204) but then 405 the real POST, which is exactly what a stale API shows. Deploy the `DSMT_Api.ps1` from this (or the 3.31.0) package to `<InstallDir>\server\DSMT_Api.ps1` and run `Restart-Service DSMT-Api` - the code itself has not changed since 3.31.0.
...
## 3.30.0
**New feature**: the console layout is now responsive below ~860px width. The sidebar (workspace switcher + nav) collapses into a hamburger-menu button in the header; tapping it slides the sidebar in as an overlay panel with a dimmed backdrop, and it auto-closes on backdrop click, on hamburger toggle, or after picking any nav item. Above the breakpoint the layout is pixel-identical to before - the sidebar stays fixed-width and always visible, same as every prior release. Implemented purely with existing patterns (inline computed styles + `componentDidMount`/`componentWillUnmount` lifecycle, matching the existing keyboard-shortcut listener) - no new CSS framework or breakpoint library added, and no page/route was touched other than the shell layout in `index.html`.
- Hot-swap: `index.html` -> the IIS webroot (e.g. `C:\inetpub\dsmt\index.html`), then hard-refresh the browser (Ctrl+F5). No service restart needed.
